...
export default function SinglePostCard({ post }: { post: Content.BlogPostDocument }) {
  return (
    <PrismicNextLink
      document={post}
      className={cn([
        "group flex w-full flex-col gap-6 rounded-lg bg-background-secondary p-3 pb-8 shadow-none",
        "transition-all duration-500 ease-kubby hover:bg-white/80 hover:shadow-expand",
        "lg:grid lg:grid-cols-2 lg:gap-2 lg:pb-4",
      ])}
    >
      ...
      <div className="_lg:px-12 flex flex-grow flex-col gap-2 justify-self-stretch px-3.5 text-primary lg:px-20 lg:py-8 xl:px-24">
        <div className="flex items-center justify-between lg:mb-4">
          ...
          <div
            className={cn([
              ...
              "text-black transition-all duration-500 ease-kubby group-hover:border-transparent",
        ...
        <div className="flex flex-grow flex-col gap-4">
          <PrismicRichText
            ...
            components={{
              heading1: ({ children }) => (
                <h1
                  className={cn([
                    "font-display font-extrabold uppercase text-primary",
                    "text-[3rem]",
                    "leading-[3.125rem]",
                    "lg:leading-[0.85] lg:[font-size:_clamp(3rem,4.76vw,5rem)]",
                  ])}
                >
                  {children}
                </h1>
          ...
          <PrismicRichText
            ...
            components={{
              paragraph: ({ children }) => <p className={"text-balance pr-2 text-base md:text-lg"}>{children}</p>,
            }}
          />
